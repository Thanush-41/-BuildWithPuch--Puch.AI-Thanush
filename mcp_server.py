...
def extract_text_from_pdf(pdf_path: str) -> str:
    ...
    try:
        ...
        raise McpError(
            ErrorData(
                code=INTERNAL_ERROR,
                message=f"Failed to read PDF file: {e!r}"
            )
        )
...
async def resume() -> str:
    ...
    Reads the PDF resume file, extracts text, and converts to markdown format.
    """
    try:
        ...
        if not resume_path.exists():
            raise McpError(
                ErrorData(
                    code=INTERNAL_ERROR,
                    message="Resume file not found at the specified path"
                )
            )
        ...
        raise McpError(
            ErrorData(
                code=INTERNAL_ERROR,
                message=f"Failed to process resume: {e!r}"
            )
        )
